...
def _officer_issue_signals(work_record: dict) -> list[dict]:
    """Translate existing analytical outputs into review signals.

    This deliberately consumes the existing component scores and explanations;
    it never changes the risk formula or turns a score into an adjudication.
    """
    signals = []
    dimensions = (
    ...
    for key, label, score_field, explanation_field, action in dimensions:
        score = pd.to_numeric(pd.Series([work_record.get(score_field)]), errors="coerce").iloc[0]
...
def _officer_filter_master(master: pd.DataFrame, state=None, constituency=None, work_status=None, severity=None, search=None):
    """Apply shared, defensive dashboard filters to the existing risk master."""
    frame = master.copy()
    if state and str(state).strip() and "state" in frame:
        frame = frame[frame["state"].fillna("").astype(str).str.casefold().eq(str(state).strip().casefold())]
    if constituency and str(constituency).strip() and "constituency" in frame:
        frame = frame[frame["constituency"].fillna("").astype(str).str.casefold().eq(str(constituency).strip().casefold())]
    if work_status and str(work_status).strip() and "work_status" in frame:
        frame = frame[frame["work_status"].fillna("").astype(str).map(_normalize_work_status).isin(_work_status_aliases(work_status))]
    if severity and str(severity).strip() and "overall_risk_level" in frame:
...
def get_officer_dashboard(
    ...
    """Officer-oriented, read-only aggregation over existing analytical data."""
    data = get_data()
...
def get_officer_work_monitoring(work_id: str = Query(..., min_length=1)):
    """Return one evidence-oriented 360° view without recalculating analytics."""
...
async def analyze_material_document(
    ...
    """Analyze uploaded material quality image/PDF document or sample voucher for price fairness."""
    file_bytes = None
...
def get_material_fairness_benchmarks(state: str = Query(None), material: str = Query(None)):
    """Return high-granularity material specification & price reference benchmarks."""
    df = load_specification_benchmarks(_BACKEND_ROOT)
    if state and state.strip():
        df = df[df["state"].str.casefold() == state.strip().casefold()]
    if material and material.strip():
        df = df[df["material"].str.casefold().str.contains(material.strip().casefold())]
    return {
        "total": len(df),
...
def get_material_sample_documents():
    """Return realistic pre-loaded SIH audit sample vouchers for interactive demo."""
    return {
...
def search_material_works(query: str = Query("", min_length=0), limit: int = 15):
    """Search real-world MPLADS projects to link with material fairness audits."""
    data = get_data()
    master = data["master"]
    ...
    if q:
        mask = (
            master["work_id"].str.lower().str.contains(q, na=False) |
            master["description"].str.lower().str.contains(q, na=False) |
            master["state"].str.lower().str.contains(q, na=False) |
            master["constituency"].str.lower().str.contains(q, na=False)
        )
        filtered = master[mask].head(limit)
    else:
        # Return prominent works with high or medium sanction costs
        filtered = master.head(limit)
    ...
    for _, row in filtered.iterrows():
        results.append({
            ...
            "sanction_amount": float(row.get("sanction_amount", 0.0)) if pd.notna(row.get("sanction_amount")) else 0.0,
            "work_category": str(row.get("work_category", "")),
            "financial_risk_score": float(row.get("financial_risk_score", 0.0)) if pd.notna(row.get("financial_risk_score")) else 0.0,
        })
    return {"total": len(results), "works": results}


@app.post("/api/material-fairness/upload-benchmark-module")
async def upload_benchmark_module(file: UploadFile = File(...)):
    """Upload custom real-world Schedule of Rates (SOR) or contractor price schedule CSV."""
    try:
        content_bytes = await file.read()
        content_str = content_bytes.decode("utf-8", errors="ignore")
        res = ingest_custom_benchmark_module(content_str)
        return res
    except Exception as e:
        return {"status": "ERROR", "message": f"Failed reading file: {str(e)}"}
