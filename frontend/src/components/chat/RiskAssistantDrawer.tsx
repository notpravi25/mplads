...
export const RiskAssistantDrawer: React.FC<RiskAssistantDrawerProps> = ({
  ...
  const handleSendPrompt = (promptText: string) => {
    ...
    setTimeout(async () => {
      ...
      if (q.includes('outlier') || q.includes('uttar pradesh') || q.includes('budget')) {
        ...
        aiResponseText = `Compliance Audit Alert: ${noPhoto.length} out of ${sampleWorks.length} sampled works lack verified geo-tagged photo proof. According to MPLADS Guidelines Section 6.2, completion stage milestone payments require visual verification prior to fund disbursement.`;
  ...
  return (
    <div className="fixed inset-0 z-[100] flex justify-end bg-black/85 backdrop-blur-md transition-opacity animate-in fade-in duration-200">
      {/* Click outside backdrop to close */}
      <div className="flex-1" onClick={onClose} />

      {/* Main Drawer Container - 100% Solid Opaque Background */}
      <div 
        style={{ backgroundColor: '#090d16', opacity: 1 }}
        className="w-full max-w-md bg-slate-950 border-l border-slate-700/80 flex flex-col h-full shadow-2xl relative z-[101] text-slate-100"
      >
        {/* Header */}
        <div style={{ backgroundColor: '#0d1322' }} className="p-4 border-b border-slate-700/80 flex items-center justify-between shrink-0">
        ...
        <div style={{ backgroundColor: '#090d16' }} className="flex-1 overflow-y-auto p-4 space-y-4 text-xs">
          {messages.map((msg) => (
            <div
              key={msg.id}
            ...
            >
              ...
              {/* Message Bubble */}
              <div
                style={msg.sender === 'ai' ? { backgroundColor: '#111726' } : {}}
                className={`p-3.5 rounded-lg max-w-[92%] leading-relaxed ${
                  msg.sender === 'user'
                    ? 'bg-blue-600 text-white rounded-br-none font-medium'
                    : 'border border-slate-700/80 text-slate-200 rounded-bl-none space-y-3'
                }`}
              >
                ...
                {msg.dataCard && (
                  <div style={{ backgroundColor: '#090d16' }} className="mt-3 p-3 rounded-md border border-slate-700/80 space-y-2 text-left">
                    ...
                    <div className="grid grid-cols-2 gap-2 pt-1">
                      {msg.dataCard.metrics.map((m, idx) => (
                        <div key={idx} style={{ backgroundColor: '#131b2c' }} className="p-2 rounded border border-slate-700/80">
                      ...
                      <div className="pt-2 border-t border-slate-800 space-y-1.5">
                        ...
                        {msg.dataCard.works.map((w) => (
                          <div
                            ...
                            className="flex items-center justify-between p-2 rounded hover:bg-slate-800 border border-slate-700/80 cursor-pointer transition-all group"
                          >
                            <div>
          ...
          {isThinking && (
            <div style={{ backgroundColor: '#111726' }} className="flex items-center gap-2 p-3 border border-slate-700/80 rounded-lg text-slate-400 text-xs w-fit">
        ...
        <div style={{ backgroundColor: '#0b0f1a' }} className="p-3 border-t border-slate-700/80 shrink-0">
            ...
            {presetQueries.map((pq, idx) => {
              ...
              return (
                <button
                  ...
                  className="flex items-center gap-2 p-2 rounded hover:bg-slate-800 border border-slate-700/80 text-left text-[11px] text-slate-300 hover:text-white transition-all disabled:opacity-50"
                >
        ...
        {/* Query Input Box */}
        <div style={{ backgroundColor: '#0d1322' }} className="p-3 border-t border-slate-700/80 flex items-center gap-2 shrink-0">
          <input
            type="text"
            value={inputQuery}
            ...
            className="flex-1 border border-slate-700 rounded-md px-3 py-2 text-xs text-slate-100 placeholder-slate-400 focus:outline-none focus:border-blue-500 transition-colors"
          />
          <button
